...
class Order(db.Model):
    ...
    status = db.Column(db.Enum('draft', 'checkout_initiated', 'awaiting_payment', 'paid', 'processing', 'awaiting_info', 'refund_required', 'ready', 'links_sent', 'completed', 'completed_partial_refund', 'cancelled_unpaid', 'cancelled_manual', 'refunded_partial', 'refunded_full', name='order_status'),
    ...
    def generate_order_number():
        ...
        for attempt in range(max_attempts):
            ...
            # Если коллизия, добавить случайный суффикс
            order_number = f'MS{timestamp}{random.randint(10, 99)}'
            if not Order.query.filter_by(order_number=order_number).first():
                return order_number
        
        # Если не получилось, использовать UUID
        import uuid
        return f'MS{uuid.uuid4().hex[:16].upper()}'
    ...
    def generate_human_order_number():
        ...
        for attempt in range(max_attempts):
            ...
            order_number = f'MS-{date_str}-{unique_id}'
            
            # ✅ Проверка уникальности
            if not Order.query.filter_by(generated_order_number=order_number).first():
                return order_number
    ...
    def can_be_captured_by_mom(self):
        ...
        # ✅ Можно принимать деньги когда ссылки отправлены или требуется частичный возврат
...
class Payment(db.Model):
    ...
    # CloudPayments data
    cp_transaction_id = db.Column(db.String(100), unique=True, nullable=True, index=True)
    amount = db.Column(db.Numeric(10, 2), nullable=False)
    currency = db.Column(db.String(3), default='RUB')
    status = db.Column(db.Enum('authorized', 'confirmed', 'voided', 'refunded_partial', 'refunded_full', 'failed', name='payment_status'),
                       default='authorized', nullable=False, index=True)
...
class AuditLog(db.Model):
    ...
    def create_log(user_id=None, action=None, resource_type=None, resource_id=None, 
                   details=None, ip_address=None, user_agent=None, commit=False):
        """
        Create audit log entry
        
        Args:
            commit: Если True, коммитит отдельно. Если False (по умолчанию), 
                    нужно коммитить вместе с основной транзакцией.
        """
        log = AuditLog(
            user_id=user_id,
            action=action,
...
class OrderChat(db.Model):
    ...
    def mark_messages_as_read(self, user_id, commit=False):
